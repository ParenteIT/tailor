...
- **Proposta em 7 blocos:** Espelho → Diagnóstico → Slider Hoje/Futuro → Fita Métrica da Virada → O Método → Prova → Oferta. A estrutura fica na holding; o desenho por vertente não foi feito (a página recebe os tokens do mundo). A voz do texto gerado ainda é a de consultora de imagem (`prompts/v1.ts`); com três vertentes, precisa de voz por vertente.
